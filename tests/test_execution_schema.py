...
def payload():
    ...
    return {
        ...
        "timestamp": "2026-08-26T09:18:00+05:30",
        ...
        "stocks": {
            "TEST": {
                ...
                "previous_day": {"high": 105, "low": 95, "close": 100},
                "volume": 300, "candles": {"1m": rows, "5m": [], "15m": [], "1h": []},
                ...
                "data_source_status": "LIVE", "last_tick": "2026-08-26T09:18:00+05:30",
            }
        },
    }
...
def test_original_payload_is_not_mutated_and_fields_unchanged():
    ...
    result = enrich_market_payload(original)
...
def test_completed_candle_detection_works():
    ...
    assert result["execution_enrichment"]["stocks"]["TEST"]["completed_candles"]["1m"]["count"] == 2
    assert result["execution_enrichment"]["stocks"]["TEST"]["completed_candles"]["1m"]["last_timestamp"] == "2026-08-26T09:16:00+05:30"


def test_candle_gaps_are_detected():
    p = payload()
    p["stocks"]["TEST"]["candles"]["1m"].append(candle("2026-08-26T09:20:00+05:30", 102, 104, 101, 103))
...
def test_timeframe_indicators_are_calculated_from_candles():
    result = enrich_market_payload(payload())
    indicators = result["execution_enrichment"]["stocks"]["TEST"]["timeframe_indicators"]["1m"]
    assert indicators["ema9"] is not None
...
def test_support_resistance_is_deterministic():
    result = enrich_market_payload(payload())
    levels = result["execution_enrichment"]["stocks"]["TEST"]["support_resistance"]["1m"]
    assert levels == {"support": 99.0, "resistance": 103.0, "method": "candle_extrema"}


def test_unavailable_data_remains_null_or_unavailable():
    p = payload()
    p["stocks"]["TEST"]["candles"] = {"1m": [], "5m": [], "15m": [], "1h": []}
    p["stocks"]["TEST"]["timestamp"] = None
    result = enrich_market_payload(p)
...
def test_opening_range_status_is_derived_without_rewriting_existing_state():
    p = payload()
    p["stocks"]["TEST"]["opening_range"] = {"period": "09:15-09:30", "status": "FORMED", "high": 103, "low": 99}
    result = enrich_market_payload(p)
...
def test_stale_timestamps_are_not_modified():
    p = payload()
    p["timestamp"] = "2026-08-26T12:00:00+05:30"
    ...
    assert result["execution_enrichment"]["stocks"]["TEST"]["data_age_seconds"] == 10080.0
...
def test_no_fabricated_values_for_missing_timeframes():
    ...
    for tf in ("5m", "15m", "1h"):
        ...
        assert e["support_resistance"][tf]["support"] is None
        assert e["support_resistance"][tf]["resistance"] is None
        assert e["timeframe_indicators"][tf]["ema9"] is None
        assert e["timeframe_indicators"][tf]["ema20"] is None
        assert e["timeframe_indicators"][tf]["vwap"] is None
