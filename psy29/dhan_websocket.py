...
def parse_quote_packet(payload: bytes, registry: InstrumentRegistry) -> QuoteTick | None:
    ...
    return QuoteTick(
        ...
        last_trade_epoch=struct.unpack_from("<i", payload, 15)[0],
        average_trade_price=struct.unpack_from("<f", payload, 19)[0],
        volume=struct.unpack_from("<i", payload, 23)[0],
        total_sell_quantity=struct.unpack_from("<i", payload, 27)[0],
        total_buy_quantity=struct.unpack_from("<i", payload, 31)[0],
        day_open=struct.unpack_from("<f", payload, 35)[0],
        day_close=struct.unpack_from("<f", payload, 39)[0],
        day_high=struct.unpack_from("<f", payload, 43)[0],
        day_low=struct.unpack_from("<f", payload, 47)[0],
    )
