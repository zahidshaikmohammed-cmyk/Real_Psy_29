...
def _patch_runner(runner: Any) -> None:
    ...
    def hardened_payload():
        ...
        if main.in_session(now) and _minute_of(now) >= 9 * 60 + 16:
            ...
            expected_first = int(now.replace(hour=9, minute=15, second=0, microsecond=0).timestamp())
...
def _patch_builder(builder: Any) -> None:
    if getattr(builder, "_psy29_builder_hardened", False):
        return

    def execution_quality(stock, candles, integrity, indicators, reference_ts):
        reasons, missing = [], []
        if builder._first(stock, "last_tick", "ltp", "last_price", "current_price") is None:
        ...
        if builder._relative_strength(stock)["status"] == "UNAVAILABLE":
        ...
        for tf in ("1m", "5m"):
            ...
            for name in ("VWAP", "EMA9", "EMA20"):
                if values.get(name, {}).get("status") == "INSUFFICIENT_DATA":
                    indicator_core = False
        has_core = builder._first(stock, "last_tick", "ltp", "last_price", "current_price") is not None and indicator_core
        integrity_gap = any(integrity[tf]["status"] == "GAP" for tf in builder._TIMEFRAMES)
        return {
            "status": "READY" if has_core and not integrity_gap else ("DEGRADED" if candles else "UNAVAILABLE"),
            "reasons": reasons,
            "missing_fields": sorted(set(missing)),
            "source_timestamp": reference_ts.isoformat() if reference_ts else None,
        }
    builder._execution_quality = execution_quality
    builder._psy29_builder_hardened = True
...
class _PostImportLoader(importlib.abc.Loader):
    ...
    def exec_module(self, module):
        ...
        elif self.fullname == "psy29.execution_master_builder":
            _patch_builder(module)
